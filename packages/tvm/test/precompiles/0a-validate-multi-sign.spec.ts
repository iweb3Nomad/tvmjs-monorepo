...
describe.each([TronMainnet, TronNile, TronShasta])(
  'Precompiles: VALIDATE-MULTI-SIGN on $name',
  (chain) => {
    it('address do not exist', async () => {
      ...
      // console.log(result)
      assert.deepEqual(result.executionGasUsed, 4500n, 'should use TRON signature energy costs')
    ...
    it('weight not enough', async () => {
      ...
      assert.deepEqual(result.executionGasUsed, 1500n, 'should use TRON signature energy costs')
      assert.deepEqual(result.returnValue, getError())
    })
    ...
    it('wrong sign', async () => {
      ...
      assert.deepEqual(result.executionGasUsed, 3000n, 'should use TRON signature energy costs')
      assert.deepEqual(result.returnValue, getError())
    })
    ...
    it('Osaka rejects invalid ABI shape and consumes the forwarded gas', async () => {
      const common = new Common({ chain, activatedProposals: [96] })
      const FUNC = getActivePrecompiles(common).get(precompileContractAddr)!
